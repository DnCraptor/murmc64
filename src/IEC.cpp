...
Drive *IEC::create_drive(unsigned num, const std::string & path)
{
...
#ifdef FRODO_RP2350
	// On RP2350, only disk images are supported (via FAT filesystem)
	int type;
	...
	the_c64->ShowNotification("Unsupported file type");
	return nullptr;
#else
...
#ifdef FRODO_RP2350
	std::string s = "Unsupported drive command '" + command + "'";
#else
...
bool IsMountableFile(const std::string & path, int & ret_type)
{
	...
	if (path.empty())
		return false;
	// On RP2350, we don't check for directories (FAT filesystem handles this differently)
#else
